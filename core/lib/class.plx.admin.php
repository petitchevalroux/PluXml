...
';
	...
	public function editStatiques($content, $action=false) {
		...
		if(!empty($content['selection']) AND $content['selection']=='delete' AND isset($content['idStatic'])) {
			foreach($content['idStatic'] as $static_id) {
				$filename = DOCUMENT_ROOT.$this->aConf['racine_statiques'].$static_id.'.'.$this->aStats[$static_id]['url'].'.php';
			...
			foreach($content['staticNum'] as $static_id) {
				...
				if($stat_name!='') {
					...
					if(isset($this->aStats[$static_id]) AND $this->aStats[$static_id]['url']!=$stat_url) {
						$oldfilename = DOCUMENT_ROOT.$this->aConf['racine_statiques'].$static_id.'.'.$this->aStats[$static_id]['url'].'.php';
						$newfilename = DOCUMENT_ROOT.$this->aConf['racine_statiques'].$static_id.'.'.$stat_url.'.php';
	...
	public function getFileStatique($num) {
		...
		$filename = DOCUMENT_ROOT.$this->aConf['racine_statiques'].$num.'.'.$this->aStats[ $num ]['url'].'.php';
	...
	public function editStatique($content) {
		...
		if($this->editStatiques(null,true)) {
			...
			$filename = DOCUMENT_ROOT.$this->aConf['racine_statiques'].$content['id'].'.'.$this->aStats[ $content['id'] ]['url'].'.php';
			# On écrit le fichier
	...
	public function editArticle($content, &$id) {
		...
		$filename = DOCUMENT_ROOT.$this->aConf['racine_articles'].$id.'.'.implode(',', $content['catId']).'.'.trim($content['author']).'.'.$time.'.'.$content['url'].'.xml';
		# On va mettre à jour notre fichier
		if(plxUtils::write($xml,$filename)) {
			# suppression ancien fichier si nécessaire
			if($oldArt) {
				$oldfilename = DOCUMENT_ROOT.$this->aConf['racine_articles'].$oldArt['0'];
	...
	public function delArticle($id) {
		...
		if($globArt = $this->plxGlob_arts->query('/^'.$id.'.(.*).xml$/')) {
			unlink(DOCUMENT_ROOT.$this->aConf['racine_articles'].$globArt['0']);
			$resDelArt = !file_exists(DOCUMENT_ROOT.$this->aConf['racine_articles'].$globArt['0']);
		}
		# Suppression des commentaires
		if($globComs = $this->plxGlob_coms->query('/^_?'.str_replace('_','',$id).'.(.*).xml$/')) {
			$nb_coms=sizeof($globComs);
			for($i=0; $i<$nb_coms; $i++) {
				unlink(DOCUMENT_ROOT.$this->aConf['racine_commentaires'].$globComs[$i]);
				$resDelCom = (!file_exists(DOCUMENT_ROOT.$this->aConf['racine_commentaires'].$globComs[$i]) AND $resDelCom);
			}
		}
	...
	public function editCommentaire($content, &$id, $editorIsAdmin) {
		...
		if(!file_exists(DOCUMENT_ROOT.$this->aConf['racine_commentaires'].$comment['filename'])) # Commentaire inexistant
		...
		$com = $this->parseCommentaire(DOCUMENT_ROOT.$this->aConf['racine_commentaires'].$comment['filename']);
	...
	public function delCommentaire($id) {
		...
		# Génération du nom du fichier
		$filename = DOCUMENT_ROOT.$this->aConf['racine_commentaires'].$id.'.xml';
	...
	public function modCommentaire(&$id, $mod) {
		...
		# Génération du nom du fichier
		$oldfilename = DOCUMENT_ROOT.$this->aConf['racine_commentaires'].$id.'.xml';
		...
		$newfilename = DOCUMENT_ROOT.$this->aConf['racine_commentaires'].$id.'.xml';
